...
export default function TopBar() {
  ...
  return (
    <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center px-4 gap-4 flex-shrink-0 z-20">
      {/* Logo & Menu */}
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => dispatch({ type: 'SET_SECTION', payload: 'mydrive' })}>
      ...
      {/* Search Bar */}
      <div className={`flex-1 max-w-2xl mx-auto relative transition-all ${searchFocused ? 'max-w-3xl' : ''}`}>
        <div className={`flex items-center gap-2 px-4 py-2.5 rounded-full transition-all ${
          searchFocused 
            ? 'bg-white dark:bg-slate-800 shadow-lg ring-2 ring-blue-500/20' 
      ...
      <div className="flex items-center gap-2">
        {/* View Toggle */}
        <div className="hidden md:flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg p-0.5">
          <button
        ...
        <button
          ...
          title="Toggle details panel (i)"
        >
          <i className="fa-solid fa-circle-info text-lg" />
        </button>

        {/* rclone Status */}
        <div className="hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
        ...
        <div className="relative" ref={accountMenuRef}>
          <button
            ...
            className="w-9 h-9 rounded-full overflow-hidden ring-2 ring-transparent hover:ring-blue-500/30 transition-all"
          >
            {currentAccount ? (
          ...
          {showAccountMenu && (
            <div className="absolute right-0 top-12 w-72 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 py-2 z-50">
                ...
                {state.accounts.map(account => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors ${
                      account.id === state.currentAccountId ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                    }`}
                  >
                    ...
                    {account.id === state.currentAccountId && (
                      <i className="fa-solid fa-check text-blue-600 dark:text-blue-400 text-xs" />
                    )}
                  </button>
                ))}
              </div>
